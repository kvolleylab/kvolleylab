...
assert.equal(d.matches.filter(m=>api.localTimeLabel(m)).length,72,'only supplied local times');
...
console.log('PASS team identity, 116 match references, 72 supplied local times, 18 roster destinations, full/link-only/none and unchanged ratios');
