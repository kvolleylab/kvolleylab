...
function addPath(svg,d){
  ...
  path.setAttribute('class','sc-ladder-line-path');
...
function drawArticle(article){
  ...
  const grid=article.querySelector(':scope>.sc-bracket-grid');
  if(!grid)return;
  let svg=grid.querySelector(':scope>.sc-ladder-lines');
  if(!svg){
    svg=document.createElementNS(SVG_NS,'svg');
    svg.setAttribute('class','sc-ladder-lines');
  ...
  const pairs=[...grid.querySelectorAll('.sc-ladder-feeder-pair')];
  const semis=[...grid.querySelectorAll('.sc-ladder-semi-stack>.sc-bracket-game')];
  const final=grid.querySelector('.sc-ladder-final-slot>.sc-bracket-game');
  if(pairs.length!==2||semis.length!==2||!final)return;

  pairs.forEach((pair,index)=>{
    const sources=[...pair.children].filter(el=>el.classList.contains('sc-bracket-game')||el.classList.contains('sc-ladder-bye'));
