...
(function(){
  ...
  async function init(){const root=document.getElementById('teamRoot'),urls=sources();if(!root||!urls.length)return;ensureStyle();addShortcut();try{const parts=await Promise.all(urls.map(u=>fetch(u,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(`${r.status} ${u}`);return r.json()})));const teams=parts.flatMap(p=>p.teams||[]);const paint=()=>{render(root,teams);openRequested(root)};paint();const observer=new MutationObserver(()=>{if(!root.querySelector('.kvl-roster-team-card'))setTimeout(paint,0)});observer.observe(root,{childList:true});}catch(err){console.error('[KVL roster]',err)}}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init,{once:true});else init();
  window.KVLCompetitionRoster=Object.freeze({version:'1.0.0',init});
})();
