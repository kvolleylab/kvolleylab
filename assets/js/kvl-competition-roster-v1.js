...
(function(){
  ...
  const STYLE_HREF='assets/css/kvl-competition-roster-v1.css?v=20260908-3';
  const ROUTES={
  ...
  const LINK_ROUTES={
    'international-competition-avc-men-continental-2026.html':'data/competitions/avc-men-continental-2026-volleybox-links.json'
  };
  ...
  const legacyLinkSource=()=>document.getElementById('teamRoot')?.dataset.kvlRosterLinks||LINK_ROUTES[currentFile()]||'';
  const flagUrl=team=>flagMap[team.code]?`https://flagcdn.com/w80/${flagMap[team.code]}.png`:'';
  const safeVolleyboxUrl=value=>{if(!value)return'';try{const u=new URL(String(value),location.href);return u.protocol==='https:'&&/(^|\.)volleybox\.net$/i.test(u.hostname)?u.href:''}catch{return''}};
  const linkMarkup=p=>{
    const url=safeVolleyboxUrl(p.volleyboxUrl);
    if(!url)return'<span class="kvl-roster-vb is-empty" aria-hidden="true"></span>';
    const label=`${p.koreanName||p.officialName||p.fullName||'선수'} Volleybox 프로필 열기`;
    return `<a class="kvl-roster-vb" href="${esc(url)}" target="_blank" rel="noopener noreferrer" aria-label="${esc(label)}"><span class="kvl-roster-vb-full">Volleybox ↗</span><span class="kvl-roster-vb-short" aria-hidden="true">VB↗</span></a>`;
  };
  const playerRow=p=>`<div class="kvl-roster-row"><span class="kvl-roster-no">${esc(p.number)}</span><div class="kvl-roster-player"><strong>${esc(p.officialName||p.fullName)}</strong><small>${esc(p.koreanName)}${p.birthDate?` · ${esc(p.birthDate)}`:''}</small></div><span class="kvl-roster-pos">${esc(p.position)}</span><span class="kvl-roster-height">${p.heightCm?`${esc(p.heightCm)}cm`:'-'}</span>${linkMarkup(p)}</div>`;
  const teamCard=team=>`<article class="kvl-roster-team-card" data-team="${esc(team.name)}"><button class="kvl-roster-team-button" type="button" aria-expanded="false"><img src="${flagUrl(team)}" alt="${esc(team.name)} 국기" loading="lazy"><span class="kvl-roster-team-copy"><strong>${esc(team.name)}</strong><small>${esc(team.en)}</small></span><span class="kvl-roster-team-meta">${esc(team.group)}조 · ${esc(team.count)}명</span><span class="kvl-roster-chevron" aria-hidden="true">⌄</span></button><div class="kvl-roster-panel"><div class="kvl-roster-head"><span>#</span><span>선수</span><span>POS</span><span>키</span><span class="kvl-roster-vb-head"><span class="kvl-roster-vb-head-full">Volleybox</span><span class="kvl-roster-vb-head-short">VB</span></span></div>${(team.players||[]).slice().sort((a,b)=>Number(a.number)-Number(b.number)).map(playerRow).join('')}<div class="kvl-roster-source-note">K-Volley Lab 선수명단 MASTER 기준 · ${esc(team.status||'등록 명단')}</div></div></article>`;
  ...
  async function fetchLinkMap(url){if(!url)return null;const r=await fetch(url,{cache:'no-store'});if(!r.ok)throw new Error(`${r.status} ${url}`);return r.json()}
  function applyLinkMap(teams,map){const indexed=map?.teams||{};return teams.map(team=>({...team,players:(team.players||[]).map(p=>({...p,volleyboxUrl:p.volleyboxUrl||indexed?.[team.code]?.[String(p.number)]||''}))}))}
  async function fetchRosterBundle(urls,linkSource){const [teams,linkMap]=await Promise.all([fetchTeams(urls),fetchLinkMap(linkSource).catch(err=>{console.warn('[KVL roster links]',err);return null})]);return applyLinkMap(teams,linkMap)}
  async function initLegacy(){const root=document.getElementById('teamRoot'),urls=legacySources();if(!root||!urls.length)return;ensureStyle();addLegacyShortcut();try{const teams=await fetchRosterBundle(urls,legacyLinkSource());const paint=()=>{render(root,teams);openRequested(root)};let tries=0;const settle=setInterval(()=>{tries++;if(!root.querySelector('.kvl-roster-team-card'))paint();if(tries>=50)clearInterval(settle)},100);paint();}catch(err){console.error('[KVL roster legacy]',err)}}
  async function initGeneric(root,cfg){if(!root||!cfg||cfg.features?.participants===false||cfg.features?.participantRosters===false)return;const urls=normalizeSources(cfg.data?.rosters);if(!urls.length)return;ensureStyle();try{const teams=await fetchRosterBundle(urls,cfg.data?.rosterLinks||'');const api=window.KVLCompetitionTemplateV1;if(!api?.register)return;api.register('participants',({slot})=>{render(slot,teams);openRequested(slot)});addGenericShortcut(root)}catch(err){console.error('[KVL roster template]',err)}}
  ...
  window.KVLCompetitionRoster=Object.freeze({version:'1.2.0',init,render});
})();