(()=>{
  ...
  Promise.all([
    ...
    const teamLink=(p,name,side,isWinner)=>`<span class="cd-team ${side}"><a href="${matchLink(p.matchId)}" target="_blank" rel="noopener noreferrer">${side==='is-left'?`<span class="avc-match-team-name ${isWinner?'cd-winner':''}">${esc(name)}</span><img class="avc-match-flag" src="${flagUrl(p.team?.code)}" alt="${esc(name)} 국기" loading="lazy">`:`<img class="avc-match-flag" src="${flagUrl(p.team?.code)}" alt="${esc(name)} 국기" loading="lazy"><span class="avc-match-team-name ${isWinner?'cd-winner':''}">${esc(name)}</span>`}</a></span>`;
    const renderStandardMatch=m=>{
      ...
      return `<article class="cd-match"><time>${esc(m.time)}</time><span class="cd-stage">${esc(stage)}</span>${teamLink({team:h,matchId:m.official_match_id},m.home,'is-left',homeWon)}<strong class="cd-score">${homeSets??'-'}-${awaySets??'-'}</strong>${teamLink({team:a,matchId:m.official_match_id},m.away,'is-right',awayWon)}${sets.length?`<div class="avc-match-sets">${sets.map((s,i)=>`<span>${i+1}세트 ${esc(s)}</span>`).join('')}</div>`:''}</article>`;
    };
    ...
    document.getElementById('avcBracket').innerHTML=`<div class="avc-bracket-main"><div class="avc-ladder-final"><div class="avc-ladder-label">결승</div>${bracketMatch(28219,'결승')}</div><span class="avc-ladder-line v-final"></span><span class="avc-ladder-line h-merge"></span><span class="avc-ladder-line v-left"></span><span class="avc-ladder-line v-right"></span><div class="avc-ladder-semi avc-ladder-semi-1"><div class="avc-ladder-label">준결승 1</div>${bracketMatch(28214,'준결승 1')}</div><div class="avc-ladder-semi avc-ladder-semi-2"><div class="avc-ladder-label">준결승 2</div>${bracketMatch(28215,'준결승 2')}</div></div><aside class="avc-bracket-side"><div class="avc-ladder-third"><div class="avc-ladder-label">3·4위 결정전</div>${bracketMatch(28218,'3·4위 결정전')}</div></aside>`;
