(()=>{
  ...
  Promise.all([
    ...
    const flagMark=(p,name)=>`<span class="cd-inline-logo"><img src="${flagUrl(p?.code)}" alt="${esc(name)} 국기" loading="lazy"></span>`;
    const renderStandardMatch=m=>{
      ...
      return `<article class="cd-match"><div class="cd-match-meta"><time>${esc(m.time)} KST</time><span>${esc(stageLabel(m))}${pool&&m.official_match_id<=28212?` · ${esc(pool)}조`:''}</span></div><div class="cd-match-board"><a class="cd-side is-left" href="${matchLink(m.official_match_id)}" target="_blank" rel="noopener noreferrer"><strong class="${homeWon?'cd-winner':''}">${esc(m.home)}</strong>${flagMark(h,m.home)}</a><b class="cd-score">${homeSets??'-'}-${awaySets??'-'}</b><a class="cd-side is-right" href="${matchLink(m.official_match_id)}" target="_blank" rel="noopener noreferrer">${flagMark(a,m.away)}<strong class="${awayWon?'cd-winner':''}">${esc(m.away)}</strong></a></div><div class="cd-set-scores">${sets.map(s=>`<span>${esc(s)}</span>`).join('')}</div></article>`;
    };
    ...
    document.getElementById('avcBracket').innerHTML=`<div class="avc-ladder-final"><div class="avc-ladder-label">결승</div>${bracketMatch(28219,'결승')}</div><div class="avc-ladder-third"><div class="avc-ladder-label">3·4위 결정전</div>${bracketMatch(28218,'3·4위 결정전')}</div><span class="avc-ladder-line v-final"></span><span class="avc-ladder-line h-merge"></span><span class="avc-ladder-line v-left"></span><span class="avc-ladder-line v-right"></span><div class="avc-ladder-semi avc-ladder-semi-1"><div class="avc-ladder-label">준결승 1</div>${bracketMatch(28214,'준결승 1')}</div><div class="avc-ladder-semi avc-ladder-semi-2"><div class="avc-ladder-label">준결승 2</div>${bracketMatch(28215,'준결승 2')}</div>`;
