...
let matches=[];
let state={week:0,places:new Set(Object.keys(META)),month:0,selectedDay:null,timeMode:'kst',mobilePanelOpen:false};
...
function filteredGames(month,day){const date=`${month.year}-${String(month.month).padStart(2,'0')}-${String(day).padStart(2,'0')}`;return matches.filter(g=>displayDate(g)===date&&(!state.week||g.week===state.week)&&state.places.has(g.place))}
...
function venueCard(p,gs){const m=META[p]||{};return `<section class="sv3-venue-card ${m.cls||''}"><div class="sv3-venue-title">${venueLabel(p)}</div>${gs.map(g=>{const t=displayTime(g),middle=g.score||'vs';return `<div class="sv3-match-row"><time>${t}</time><div class="sv3-result-line"><div class="sv3-team-block">${flagImg(g.home)}<span class="sv3-team-name">${g.home}</span></div><strong class="sv3-result-score">${middle}</strong><div class="sv3-team-block"><span class="sv3-team-name">${g.away}</span>${flagImg(g.away)}</div></div></div>`}).join('')}</section>`}
function bind(){
 ...
 document.getElementById('sv3Reset').onclick=()=>{state={week:0,places:new Set(Object.keys(META)),month:0,selectedDay:null,timeMode:'kst',mobilePanelOpen:true};render()}
}
