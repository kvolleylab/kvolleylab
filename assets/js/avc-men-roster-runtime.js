(()=>{
  ...
  const clubLabel=(clubs,teamCode,number)=>{
    ...
    if(rec.status==='CONFIRMED')return rec.club||fallback;
    if(rec.status==='FREE_AGENT')return 'FA / 무소속';
    return fallback;
  };
  ...
  const buildShell=()=>{
    ...
    if(!document.getElementById('players')){
      const section=document.createElement('section');section.id='players';section.className='avc-section avc-view';section.innerHTML=`<div class="avc-section-head"><div><p class="eyebrow">OFFICIAL ROSTERS</p><h2>선수명단</h2></div><p>국가별 최종 엔트리 · 2026-27 소속팀 · VB 바로가기</p></div><div id="avcRosterRoot" class="avc-roster-loading">선수명단 불러오는 중…</div>`;sources.insertAdjacentElement('beforebegin',section);
    }
    return true;
  };
  ...
  const playerCard=(p,teamCode,clubs)=>{
    ...
    const club=clubLabel(clubs,teamCode,p.number);
