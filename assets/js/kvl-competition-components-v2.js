...
function matchRow(m){const s=score(m),detail=[stage(m),m.group?groupLabel(m.group):'',venue(m),m.localTimeLabel||''].filter(Boolean).join(' · ');return `<article data-match-id="${esc(mid(m))}" class="kvl1180-match-row ${[code(m.home),code(m.away)].includes(data().focusTeamCode)?'is-korea':''}"><div class="kvl1180-match-meta"><time>${esc(time(m)||'시간 미정')} KST</time></div><div class="kvl1180-match-board">${side(m.home,'left')}<b class="kvl1180-score ${s?'':'is-upcoming'}">${s?`${s.home}-${s.away}`:'VS'}</b>${side(m.away,'right')}</div><div class="kvl1180-set-scores">${setLine(m)}</div><div class="kvl1180-match-detail">${esc(detail)}</div></article>`;}
...
function pcard(p){const tag=p.url?'a':'button',attrs=p.url?` href="${esc(p.url)}"`:` type="button"`,count=p.count??p.rosterCount;return `<${tag} class="kvl1180-participant-button" data-team-code="${esc(p.code||'')}"${attrs}>${p.flag?`<span class="kvl-participant-flag"><img src="${esc(p.flag)}" alt="${esc(p.name||'')} 국기" loading="lazy"></span>`:'<span class="kvl-participant-flag" aria-hidden="true"></span>'}<span class="kvl1180-participant-button-copy"><strong>${esc(p.name||'미정')}</strong><small>${esc(p.en||p.code||'')}${count!==undefined?` · ${esc(count)}명`:''}</small></span></${tag}>`;}
