...
function teamCell(t,cls){const english=en(t)||code(t);return `<span class="${cls}">${logo(t)?teamLink(t,`<img src="${esc(logo(t))}" alt="${esc(name(t))} 엠블럼" loading="lazy">`,'kvl-team-flag-link'):''}<span class="${cls}-copy"><strong>${teamLink(t)}</strong><small>${teamLink(t,esc(english),'kvl-team-en-link')}</small></span></span>`;}
...
function renderFinal(d){
  ...
  top.innerHTML=rows.map(x=>`<article data-final-rank="${esc(x.rank)}" class="kvl1180-final-card ${Number(x.rank)===1?'is-champion':''}"><span class="kvl1180-final-rank" data-rank="${esc(x.rank)}">${esc(x.rank)}위</span>${x.flag?`<span class="kvl1180-final-flag">${teamLink(x,`<img src="${esc(x.flag)}" alt="${esc(x.team||'')} 국기" loading="lazy">`,'kvl-team-flag-link')}</span>`:'<span class="kvl1180-final-flag is-pending">?</span>'}<div class="kvl1180-final-copy"><strong>${teamLink(x,esc(x.team||'미정'))}</strong><small><span class="kvl-final-result">${esc(x.result||'')}</span>${x.qualification?`<span class="kvl-final-qualification">${esc(x.qualification)}</span>`:''}</small></div></article>`).join('');
}
...
function pcard(p){const action=rosterAction(data(),p),count=p.count??p.rosterCount,available=action.available;
 if(available&&action.mode==='full')return `<button type="button" class="kvl1180-participant-button" data-team-code="${esc(p.code||'')}" data-roster-internal="true" aria-pressed="false">${(p.logo||p.flag)?`<span class="kvl-participant-flag"><img src="${esc(p.logo||p.flag)}" alt="${esc(p.name||'')} 엠블럼" loading="lazy"></span>`:'<span class="kvl-participant-flag" aria-hidden="true"></span>'}<span class="kvl1180-participant-button-copy"><strong>${esc(p.name||'미정')}</strong><small>${esc(p.en||p.code||'')}${count!==undefined?` · ${esc(count)}명`:''}</small></span></button>`;
 if(action.href)return `<a class="kvl1180-participant-button" data-team-code="${esc(p.code||'')}" href="${esc(action.href)}">${(p.logo||p.flag)?`<span class="kvl-participant-flag"><img src="${esc(p.logo||p.flag)}" alt="${esc(p.name||'')} 엠블럼" loading="lazy"></span>`:'<span class="kvl-participant-flag" aria-hidden="true"></span>'}<span class="kvl1180-participant-button-copy"><strong>${esc(p.name||'미정')}</strong><small>${esc(p.en||p.code||'')}${count!==undefined?` · ${esc(count)}명`:''}</small></span></a>`;
 return `<button type="button" class="kvl1180-participant-button" data-team-code="${esc(p.code||'')}" disabled aria-disabled="true" title="등록 선수명단 미제공">${(p.logo||p.flag)?`<span class="kvl-participant-flag"><img src="${esc(p.logo||p.flag)}" alt="${esc(p.name||'')} 엠블럼" loading="lazy"></span>`:'<span class="kvl-participant-flag" aria-hidden="true"></span>'}<span class="kvl1180-participant-button-copy"><strong>${esc(p.name||'미정')}</strong><small>${esc(p.en||p.code||'')}${count!==undefined?` · ${esc(count)}명`:''} · 명단 미제공</small></span></button>`;
}
