...
function renderFinal(d){
  ...
  top.innerHTML=rows.map(x=>`<article data-final-rank="${esc(x.rank)}" class="kvl1180-final-card ${Number(x.rank)===1?'is-champion':''}"><span class="kvl1180-final-rank" data-rank="${esc(x.rank)}">${esc(x.rank)}위</span>${x.flag?`<span class="kvl1180-final-flag"><img src="${esc(x.flag)}" alt="${esc(x.team||'')} 국기"></span>`:'<span class="kvl1180-final-flag is-pending">?</span>'}<div class="kvl1180-final-copy"><strong>${esc(x.team||'미정')}</strong><small><span class="kvl-final-result">${esc(x.result||'')}</span>${x.qualification?`<span class="kvl-final-qualification">${esc(x.qualification)}</span>`:''}</small></div></article>`).join('');
}
