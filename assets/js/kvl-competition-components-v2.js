...
function pcard(p){
 ...
 if(domestic){
   const school=p.fullName||p.name||'',href='university-team.html?school='+encodeURIComponent(school)+'&view=roster',rosterLabel=count!==undefined?`등록선수명단 ${esc(count)}명`:'등록선수명단 보기';
   return `<a class="kvl1180-participant-button" data-team-code="${esc(p.code||'')}" href="${esc(href)}" aria-label="${esc(p.name||school||'대학')} 등록선수명단 보기">${asset?`<span class="kvl-participant-flag"><img src="${esc(asset)}" alt="${esc(p.name||'')} 엠블럼" loading="lazy"></span>`:`<span class="kvl-participant-flag is-text-mark" aria-hidden="true"><b>${esc(schoolMark(p.name))}</b></span>`}<span class="kvl1180-participant-button-copy"><strong>${esc(p.name||'미정')}</strong><span class="kvl-domestic-roster-cta">${rosterLabel}<b aria-hidden="true">→</b></span></span></a>`;
 }
