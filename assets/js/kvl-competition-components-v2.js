...
function domesticOverallMobileLine(r){const raw=r.team||{},t=participant(raw)||raw;return `<div class="kvl-shared-combined-line"><span class="kvl-shared-combined-identity"><span class="kvl-shared-combined-rank">${esc(r.overallRank??'-')}위</span><strong class="kvl-shared-combined-team">${teamLink(t)}</strong></span><span class="kvl-shared-combined-stat">${esc(groupLabel(r.poolId))}</span><span class="kvl-shared-combined-stat">${esc(r.games)}</span><span class="kvl-shared-combined-stat">${esc(r.wins??0)}</span><span class="kvl-shared-combined-stat">${esc(r.losses??0)}</span><span class="kvl-shared-combined-stat">${esc(r.winRateLabel)}</span><span class="kvl-shared-combined-stat">${esc(r.setRatio??'-')}</span><span class="kvl-shared-combined-stat">${esc(r.pointRatio??'-')}</span></div>`;}
function domesticOverallBlock(d,rows,c){
  const title=c.overallTitle||'예선 종합순위';
  return `<section class="kvl1180-combined-block is-domestic-overall"><div class="kvl1180-combined-head"><div><p class="label">PRELIMINARY OVERALL</p><h3>${esc(title)}</h3></div><p>승률 → 세트 득실비 → 점수 득실비</p></div><div class="kvl1180-combined-table"><div class="kvl1180-combined-table-head"><span>순위</span><span>팀</span><span>조</span><span>경기</span><span>승</span><span>패</span><span>승률</span><span>세트 득실비</span><span>점수 득실비</span></div>${rows.map(r=>`<div class="kvl1180-combined-row is-domestic-overall-row"><span class="kvl1180-combined-rank">${esc(r.overallRank??'-')}위</span>${domesticOverallTeam(r.team||{})}<span class="kvl1180-combined-pool">${esc(groupLabel(r.poolId))}</span><span class="kvl1180-combined-stat">${esc(r.games)}</span><span class="kvl1180-combined-stat">${esc(r.wins??0)}</span><span class="kvl1180-combined-stat">${esc(r.losses??0)}</span><span class="kvl1180-combined-stat">${esc(r.winRateLabel)}</span><span class="kvl1180-combined-stat">${esc(r.setRatio??'-')}</span><span class="kvl1180-combined-stat">${esc(r.pointRatio??'-')}</span></div>`).join('')}</div><div class="kvl-shared-combined-mobile is-domestic-overall-mobile"><div class="kvl-shared-combined-table"><div class="kvl-shared-combined-line is-head"><span class="kvl-shared-combined-identity"><span class="kvl-shared-combined-rank-head">순위</span><span class="kvl-shared-combined-country-head">팀</span></span><span>조</span><span>경기</span><span>승</span><span>패</span><span>승률</span><span>세트 득실비</span><span>점수 득실비</span></div>${rows.map(domesticOverallMobileLine).join('')}</div></div><p class="kvl1180-groups-footnote">예선 종합순위는 조별 진출 순위와 별개인 K-Volley Lab 산출 순위입니다. 조별 경기 수가 다른 경우 승률을 우선 적용합니다.</p></section>`;
}
...
function bbye(t,seeds){if(!t)return '';const p=participant(t)||t,domestic=String(data().competitionFamily||data().family||'').toLowerCase()==='domestic',asset=logo(p),k=[code(p),en(p),name(p)].find(x=>seeds.has(x)),seed=k?seeds.get(k):null,english=domestic?'':(en(p)||code(p)||''),mark=asset?`<span class="kvl1180-bracket-mark">${teamLink(p,`<img src="${esc(asset)}" alt="${esc(name(p))} 엠블럼" loading="lazy">`,'kvl-team-flag-link')}</span>`:`<span class="kvl1180-bracket-mark is-path ${domestic?'is-school-mark':''}">${esc(domestic?schoolMark(name(p)):'↗')}</span>`;return `<article class="kvl1180-match is-bye"><div class="kvl1180-match-head"><strong>준결승 직행</strong><span>${seed!=null?`예선 ${esc(seed)}위`:'직행'}</span></div><div class="kvl1180-bracket-team is-winner">${mark}<span class="kvl1180-bracket-copy"><strong>${teamLink(p)}</strong>${english?`<small>${esc(english)}</small>`:''}</span></div></article>`;}
