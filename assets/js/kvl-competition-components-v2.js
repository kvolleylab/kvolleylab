...
function bindKpis(d){
  const targets=[d.structure?.kpiTargets?.participants||`?view=rosters${d.focusTeamCode?'&team='+encodeURIComponent(d.focusTeamCode):''}`,'?view=groups','?view=schedule','?view=knockout'];
...
function side(t,pos){const mark=flag(t)?`<span class="kvl1180-inline-logo"><img src="${esc(flag(t))}" alt="${esc(name(t))} 국기" loading="lazy"></span>`:`<span class="kvl1180-inline-logo kvl1180-seed-icon">${esc(code(t)||'?')}</span>`,copy=`<span class="kvl1180-slot-copy"><strong>${esc(name(t))}</strong></span>`;return pos==='left'?`<span class="kvl1180-side is-left">${mark}${copy}</span>`:`<span class="kvl1180-side is-right">${copy}${mark}</span>`;}
function matchRow(m){const s=score(m),detail=[stage(m),m.group?`${m.group}조`:'',venue(m),m.localTimeLabel||''].filter(Boolean).join(' · ');return `<article class="kvl1180-match-row ${[code(m.home),code(m.away)].includes(data().focusTeamCode)?'is-korea':''}"><div class="kvl1180-match-meta"><time>${esc(time(m)||'시간 미정')} KST</time></div><div class="kvl1180-match-board">${side(m.home,'left')}<b class="kvl1180-score ${s?'':'is-upcoming'}">${s?`${s.home}-${s.away}`:'VS'}</b>${side(m.away,'right')}</div><div class="kvl1180-set-scores">${setLine(m)}</div><div class="kvl1180-match-detail">${esc(detail)}</div></article>`;}
function renderSchedule(d){
  ...
  const pools=[...new Set((d.matches||[]).filter(m=>stage(m)===runtime.stage).map(m=>m.group).filter(Boolean))];
  ...
  let list=(d.matches||[]).slice();if(runtime.pool!=='전체')list=list.filter(m=>m.group===runtime.pool);if(runtime.stage!=='전체')list=list.filter(m=>stage(m)===runtime.stage);list.sort((a,b)=>`${date(a)}T${time(a)}`.localeCompare(`${date(b)}T${time(b)}`));
...
function teamCell(t,cls){return `<span class="${cls}">${flag(t)?`<img src="${esc(flag(t))}" alt="${esc(name(t))} 국기" loading="lazy">`:''}<span class="${cls}-copy"><strong>${esc(name(t))}</strong><small>${esc(code(t)||en(t))}</small></span></span>`;}
...
function poolCard(p){const rows=p.rows||[];return `<article class="kvl1180-pool-card"><header class="kvl1180-pool-card-head"><strong>${esc(p.title||`${p.id||''}조`)}</strong><span>${rows.length}개국${p.matchCount!==undefined?' · '+esc(p.matchCount)+'경기':''}</span></header><div class="kvl1180-pool-table-head"><span>순위</span><span>국가</span><span>승</span><span>패</span><span>승점</span><span>세트율</span><span>득점율</span></div>${rows.map(poolRow).join('')}</article>`;}
...
function combinedRow(d,r,c){const st=statusOf(r),t=r.team||{},pool=poolRankLabel(d,r),pair=qfPair(r,c);return `<div class="kvl1180-combined-row ${st.cls}"><span class="kvl1180-combined-rank">${esc(r.rank??'-')}위</span>${teamCell(t,'kvl1180-combined-team')}<span class="kvl1180-combined-pool">${esc(pool)}</span><span class="kvl1180-combined-stat">${esc(r.wins??'-')}</span><span class="kvl1180-combined-stat">${esc(r.losses??'-')}</span><span class="kvl1180-combined-stat">${esc(r.points??'-')}</span><span class="kvl1180-combined-stat">${esc(r.setRatio??'-')}</span><span class="kvl1180-combined-stat">${esc(r.pointRatio??'-')}</span><span><b class="kvl1180-combined-result ${st.cls}">${esc(st.label||'')}</b>${pair?`<small class="kvl1180-qf-pairing">${esc(pair)}</small>`:''}</span></div>`;}
function mobileCombinedLine(d,r,c){const st=statusOf(r),t=r.team||{},pair=qfPair(r,c);return `<div class="kvl-shared-combined-line ${code(t)===d.focusTeamCode?'is-korea':''}"><span class="kvl-shared-combined-identity"><span class="kvl-shared-combined-rank">${esc(r.rank??'-')}위</span>${flag(t)?`<span class="kvl-shared-combined-flag"><img src="${esc(flag(t))}" alt="${esc(name(t))} 국기"></span>`:''}<strong class="kvl-shared-combined-team">${esc(name(t))}</strong></span><span class="kvl-shared-combined-result ${st.cls}"><b>${esc(st.label||'')}</b>${pair?`<small>${esc(pair)}</small>`:''}</span><span class="kvl-shared-combined-stat">${esc(r.wins??'-')}승</span><span class="kvl-shared-combined-stat">${esc(r.points??'-')}</span><span class="kvl-shared-combined-stat">${esc(r.setRatio??'-')}</span><span class="kvl-shared-combined-stat">${esc(r.pointRatio??'-')}</span><span class="kvl-shared-combined-stat">${esc(poolRankLabel(d,r))}</span></div>`;}
...
function bteam(m,key,seeds){const t=key==='home'?m.home:m.away,s=score(m),won=!!winner(m)&&name(winner(m))===name(t),k=[code(t),en(t),name(t)].find(x=>seeds.has(x)),seed=k?seeds.get(k):null,sc=key==='home'?s?.home:s?.away;return `<div class="kvl1180-bracket-team ${won?'is-winner':''}">${flag(t)?`<span class="kvl1180-bracket-mark"><img src="${esc(flag(t))}" alt="${esc(name(t))} 국기" loading="lazy"></span>`:`<span class="kvl1180-bracket-mark is-path">?</span>`}<span class="kvl1180-bracket-copy"><strong>${esc(name(t))}</strong><small>${seed!=null&&Number.isFinite(Number(seed))&&round(m)==='QF'?`예선 ${seed}번 시드`:esc(code(t)||en(t)||'대진')}</small></span><b class="kvl1180-team-score">${sc??'-'}</b></div>`;}
...
function renderRoster(d,teamCode){
 ...
 if(c.mode==='none'){root.innerHTML='';return;}
 if(c.mode==='link-only'){root.innerHTML=t?.url?`<a class="kvl1180-roster-pending" href="${esc(t.url)}">${esc(t.name)} 선수명단 보기 →</a>`:`<p class="kvl1180-roster-pending">${esc(d.rosterPendingLabel||'등록 선수명단은 공식 자료 확인 후 연결합니다.')}</p>`;return;}
