...
function side(t,pos){const p=participant(t)||t,domestic=String(data().competitionFamily||data().family||'').toLowerCase()==='domestic',asset=logo(p),mark=asset?`<span class="kvl1180-inline-logo">${teamLink(p,`<img src="${esc(asset)}" alt="${esc(name(p))} 엠블럼" loading="lazy">`,'kvl-team-flag-link')}</span>`:`<span class="kvl1180-inline-logo kvl1180-seed-icon">${esc(code(p)||'?')}</span>`,english=domestic?'':(en(p)||code(p)),copy=`<span class="kvl1180-slot-copy"><strong>${teamLink(p)}</strong>${english?`<small>${teamLink(p,esc(english),'kvl-team-en-link')}</small>`:''}</span>`;return pos==='left'?`<span class="kvl1180-side is-left">${mark}${copy}</span>`:`<span class="kvl1180-side is-right">${copy}${mark}</span>`;}
function matchRow(m){
...
function bteam(m,key,seeds){const raw=key==='home'?m.home:m.away,t=participant(raw)||raw,domestic=String(data().competitionFamily||data().family||'').toLowerCase()==='domestic',s=score(m),w=winner(m),wp=participant(w)||w,won=!!wp&&code(wp)===code(t),k=[code(t),en(t),name(t)].find(x=>seeds.has(x)),seed=k?seeds.get(k):null,sc=key==='home'?s?.home:s?.away,english=en(t)||code(t)||'대진',seedText=seed!=null&&Number.isFinite(Number(seed))&&round(m)==='QF'?`예선 ${seed}번 시드`:'',meta=domestic?seedText:[english,seedText].filter(Boolean).join(' · ');return `<div class="kvl1180-bracket-team ${won?'is-winner':''}">${logo(t)?`<span class="kvl1180-bracket-mark">${teamLink(t,`<img src="${esc(logo(t))}" alt="${esc(name(t))} 엠블럼" loading="lazy">`,'kvl-team-flag-link')}</span>`:`<span class="kvl1180-bracket-mark is-path">?</span>`}<span class="kvl1180-bracket-copy"><strong>${teamLink(t)}</strong>${meta?`<small>${esc(meta)}</small>`:''}</span><b class="kvl1180-team-score">${sc??'-'}</b></div>`;}
...
function bbye(t,seeds){if(!t)return '';const p=participant(t)||t,domestic=String(data().competitionFamily||data().family||'').toLowerCase()==='domestic',asset=logo(p),k=[code(p),en(p),name(p)].find(x=>seeds.has(x)),seed=k?seeds.get(k):null,english=domestic?'':(en(p)||code(p)||'');return `<article class="kvl1180-match is-bye"><div class="kvl1180-match-head"><strong>준결승 직행</strong><span>${seed!=null?`예선 ${esc(seed)}위`:'직행'}</span></div><div class="kvl1180-bracket-team is-winner">${asset?`<span class="kvl1180-bracket-mark">${teamLink(p,`<img src="${esc(asset)}" alt="${esc(name(p))} 엠블럼" loading="lazy">`,'kvl-team-flag-link')}</span>`:'<span class="kvl1180-bracket-mark is-path">↗</span>'}<span class="kvl1180-bracket-copy"><strong>${teamLink(p)}</strong>${english?`<small>${esc(english)}</small>`:''}</span></div></article>`;}
