...
function teamLink(t,content=esc(name(t))){const p=participant(t);return p?`<a class="kvl-team-link" data-team-results="${esc(p.code)}" href="${esc(viewUrl('team',{team:p.code}))}" aria-label="${esc(name(p))} 경기결과">${content}</a>`:content;}
...
function side(t,pos){const mark=flag(t)?`<span class="kvl1180-inline-logo"><img src="${esc(flag(t))}" alt="${esc(name(t))} 국기" loading="lazy"></span>`:`<span class="kvl1180-inline-logo kvl1180-seed-icon">${esc(code(t)||'?')}</span>`,copy=`<span class="kvl1180-slot-copy"><strong>${teamLink(t)}</strong></span>`;return pos==='left'?`<span class="kvl1180-side is-left">${mark}${copy}</span>`:`<span class="kvl1180-side is-right">${copy}${mark}</span>`;}
...
function renderTeamResults(d){
 ...
 root.innerHTML=`<div class="kvl1180-section-head kvl-team-results-head"><div class="kvl-team-results-identity">${flag(t)?`<img src="${esc(flag(t))}" alt="${esc(name(t))} 국기">`:''}<div><p class="label">TEAM RESULTS · ${esc(t.code)}</p><h2>${esc(name(t))} 경기결과</h2><p class="kvl-team-competition">${esc(d.displayName)}</p></div></div><div class="kvl-team-actions">${rosterButton(d,t)}<a href="${esc(viewUrl('schedule'))}">전체 경기일정 보기</a></div></div><div class="kvl1180-schedule-summary"><strong>${esc(name(t))} · 전체 ${list.length}경기</strong><span>완료 ${done.length} · ${wins}승 ${done.length-wins}패 · 예정 ${list.length-done.length} · 한국시간(KST)</span></div><div class="kvl1180-schedule-list">${matchGroups(list,'team')}</div>`;
}
...
function teamCell(t,cls){return `<span class="${cls}">${flag(t)?`<img src="${esc(flag(t))}" alt="${esc(name(t))} 국기" loading="lazy">`:''}<span class="${cls}-copy"><strong>${teamLink(t)}</strong><small>${esc(code(t)||en(t))}</small></span></span>`;}
...
function poolCard(p){const rows=p.rows||[];return `<article class="kvl1180-pool-card"><header class="kvl1180-pool-card-head"><strong>${esc(p.title||`${p.id||''}조`)}</strong><span>${rows.length}개국${p.matchCount!==undefined?' · '+esc(p.matchCount)+'경기':''}</span></header><div class="kvl1180-pool-table-head"><span>순위</span><span>국가</span><span>승</span><span>패</span><span>승점</span><span>세트 득실비</span><span>점수 득실비</span></div>${rows.map(poolRow).join('')}</article>`;}
...
function renderFinal(d){
  ...
  top.innerHTML=rows.map(x=>`<article data-final-rank="${esc(x.rank)}" class="kvl1180-final-card ${Number(x.rank)===1?'is-champion':''}"><span class="kvl1180-final-rank" data-rank="${esc(x.rank)}">${esc(x.rank)}위</span>${x.flag?`<span class="kvl1180-final-flag"><img src="${esc(x.flag)}" alt="${esc(x.team||'')} 국기"></span>`:'<span class="kvl1180-final-flag is-pending">?</span>'}<div class="kvl1180-final-copy"><strong>${teamLink(x,esc(x.team||'미정'))}</strong><small><span class="kvl-final-result">${esc(x.result||'')}</span>${x.qualification?`<span class="kvl-final-qualification">${esc(x.qualification)}</span>`:''}</small></div></article>`).join('');
}
...
function bteam(m,key,seeds){const t=key==='home'?m.home:m.away,s=score(m),won=!!winner(m)&&name(winner(m))===name(t),k=[code(t),en(t),name(t)].find(x=>seeds.has(x)),seed=k?seeds.get(k):null,sc=key==='home'?s?.home:s?.away;return `<div class="kvl1180-bracket-team ${won?'is-winner':''}">${flag(t)?`<span class="kvl1180-bracket-mark"><img src="${esc(flag(t))}" alt="${esc(name(t))} 국기" loading="lazy"></span>`:`<span class="kvl1180-bracket-mark is-path">?</span>`}<span class="kvl1180-bracket-copy"><strong>${teamLink(t)}</strong><small>${seed!=null&&Number.isFinite(Number(seed))&&round(m)==='QF'?`예선 ${seed}번 시드`:esc(code(t)||en(t)||'대진')}</small></span><b class="kvl1180-team-score">${sc??'-'}</b></div>`;}
