...
const schoolMark=n=>String(n||'').replace(/대학교/g,'').replace(/대$/,'').replace(/\s+/g,'').slice(0,2)||'?';
...
function side(t,pos){const p=participant(t)||t,domestic=String(data().competitionFamily||data().family||'').toLowerCase()==='domestic',asset=logo(p),mark=asset?`<span class="kvl1180-inline-logo">${teamLink(p,`<img src="${esc(asset)}" alt="${esc(name(p))} 엠블럼" loading="lazy">`,'kvl-team-flag-link')}</span>`:`<span class="kvl1180-inline-logo kvl1180-seed-icon ${domestic?'is-school-mark':''}">${esc(domestic?schoolMark(name(p)):code(p)||'?')}</span>`,english=domestic?'':(en(p)||code(p)),copy=`<span class="kvl1180-slot-copy"><strong>${teamLink(p)}</strong>${english?`<small>${teamLink(p,esc(english),'kvl-team-en-link')}</small>`:''}</span>`;return pos==='left'?`<span class="kvl1180-side is-left">${mark}${copy}</span>`:`<span class="kvl1180-side is-right">${copy}${mark}</span>`;}
function matchRow(m){
 ...
 const timeText=domestic?`${time(m)||'시간 미정'}`:`${time(m)||'시간 미정'} KST`;
...
function renderTeamResults(d){
 ...
 const summary=domestic?`<div class="kvl-team-record-wrap"><div class="kvl-team-record-summary" aria-label="${esc(name(t))} 대회 성적 요약"><article><span>전체 경기</span><strong>${list.length}<small>경기</small></strong></article><article class="is-wins"><span>승</span><strong>${wins}<small>승</small></strong></article><article class="is-losses"><span>패</span><strong>${losses}<small>패</small></strong></article><article class="is-rate"><span>승률</span><strong>${winRate}<small>%</small></strong></article></div>${pending?`<p class="kvl-team-record-note">완료 ${done.length}경기 · 예정 ${pending}경기 · 한국시간(KST)</p>`:`<p class="kvl-team-record-note">전체 일정 완료 · 한국시간(KST)</p>`}</div>`:`<div class="kvl1180-schedule-summary"><strong>${esc(name(t))} · 전체 ${list.length}경기</strong><span>완료 ${done.length} · ${wins}승 ${losses}패 · 예정 ${pending} · 한국시간(KST)</span></div>`;
 root.innerHTML=`<div class="kvl1180-section-head kvl-team-results-head">${rosterIdentity(d,t,identity)}<div class="kvl-team-actions">${rosterButton(d,t)}<a href="${esc(viewUrl('schedule'))}">전체 경기일정 보기</a></div></div>${summary}<div class="kvl1180-schedule-list">${matchGroups(list,'team')}</div>`;
...
function teamCell(t,cls){const p=participant(t)||t,domestic=String(data().competitionFamily||data().family||'').toLowerCase()==='domestic',english=domestic?'':(en(p)||code(p));return `<span class="${cls}">${logo(p)?teamLink(p,`<img src="${esc(logo(p))}" alt="${esc(name(p))} 엠블럼" loading="lazy">`,'kvl-team-flag-link'):''}<span class="${cls}-copy"><strong>${teamLink(p)}</strong>${english?`<small>${teamLink(p,esc(english),'kvl-team-en-link')}</small>`:''}</span></span>`;}
function poolRow(r){const st=statusOf(r),t=r.team||{},domestic=String(data().competitionFamily||data().family||'').toLowerCase()==='domestic',p=participant(t)||t,team=domestic?`<span class="kvl1180-pool-team kvl1180-pool-team-text"><span class="kvl1180-pool-team-copy"><strong>${teamLink(p)}</strong></span></span>`:teamCell(t,'kvl1180-pool-team');return `<div class="kvl1180-pool-row ${st.cls}"><span class="kvl1180-pool-rank">${esc(r.rank??'-')}</span>${team}<span class="kvl1180-pool-w">${esc(r.wins??'-')}</span><span class="kvl1180-pool-l">${esc(r.losses??'-')}</span><span class="kvl1180-pool-pts">${esc(r.points??'-')}</span><span class="kvl1180-pool-ratio">${esc(r.setRatio??'-')}</span><span class="kvl1180-pool-ratio">${esc(r.pointRatio??'-')}</span></div>`;}
function poolCard(p){const rows=p.rows||[],domestic=String(data().competitionFamily||data().family||'').toLowerCase()==='domestic';return `<article class="kvl1180-pool-card"><header class="kvl1180-pool-card-head"><strong>${esc(p.title||`${p.id||''}조`)}</strong><span>${rows.length}${domestic?'팀':'개국'}${p.matchCount!==undefined?' · '+esc(p.matchCount)+'경기':''}</span></header><div class="kvl1180-pool-table-head"><span>순위</span><span>${domestic?'팀':'국가'}</span><span>승</span><span>패</span><span>승점</span>${domestic?'<span class="kvl-ratio-head"><b>세트</b><small>득실비</small></span><span class="kvl-ratio-head"><b>점수</b><small>득실비</small></span>':'<span>세트 득실비</span><span>점수 득실비</span>'}</div>${rows.map(poolRow).join('')}</article>`;}
...
function renderDomesticRuleCard(d,key,anchor){
  if(!anchor)return;
  const domestic=String(d.competitionFamily||d.family||'').toLowerCase()==='domestic',spec=d.domesticRules?.[key],selector=`[data-kvl-domestic-rule="${key}"]`;
  const existing=anchor.parentElement?.querySelector(selector);
  if(!domestic||!spec){if(existing)existing.remove();return;}
  const el=existing||document.createElement('aside');el.className='kvl1180-domestic-rule-card';el.dataset.kvlDomesticRule=key;
  el.innerHTML=`<p><strong>${esc(spec.title||'대회 운영 기준')}</strong>${esc(spec.body||'')}</p>${spec.emphasis?`<b>${esc(spec.emphasis)}</b>`:''}${spec.note?`<small>${esc(spec.note)}</small>`:''}${spec.sourceUrl?`<a href="${esc(spec.sourceUrl)}" target="_blank" rel="noopener noreferrer">대회요강 원문 확인 →</a>`:''}`;
...
function renderFinal(d){
  ...
  const domestic=String(d.competitionFamily||d.family||'').toLowerCase()==='domestic';top.innerHTML=rows.map(x=>{const p=participant(x.code)||x,asset=logo(p),fallback=domestic?`<span class="kvl1180-final-flag is-text-mark"><b>${esc(schoolMark(x.team||name(p)))}</b></span>`:'<span class="kvl1180-final-flag is-pending">?</span>';return `<article data-final-rank="${esc(x.rank)}" class="kvl1180-final-card rank-${esc(x.rank)} ${Number(x.rank)===1?'is-champion':''}"><span class="kvl1180-final-rank" data-rank="${esc(x.rank)}">${medal(x.rank)?`<span class="kvl-final-medal" aria-hidden="true">${medal(x.rank)}</span>`:''}<b>${esc(x.rank)}위</b></span>${asset?`<span class="kvl1180-final-flag">${teamLink(p,`<img src="${esc(asset)}" alt="${esc(x.team||name(p)||'')} 엠블럼" loading="lazy">`,'kvl-team-flag-link')}</span>`:fallback}<div class="kvl1180-final-copy"><strong>${teamLink(p,esc(x.team||name(p)||'미정'))}</strong><small><span class="kvl-final-result">${esc(x.result||'')}</span>${x.qualification?`<span class="kvl-final-qualification">${esc(x.qualification)}</span>`:''}</small></div></article>`}).join('');
}
...
function bteam(m,key,seeds){const raw=key==='home'?m.home:m.away,t=participant(raw)||raw,domestic=String(data().competitionFamily||data().family||'').toLowerCase()==='domestic',s=score(m),w=winner(m),wp=participant(w)||w,won=!!wp&&code(wp)===code(t),k=[code(t),en(t),name(t)].find(x=>seeds.has(x)),seed=k?seeds.get(k):null,sc=key==='home'?s?.home:s?.away,english=en(t)||code(t)||'대진',seedText=seed!=null&&Number.isFinite(Number(seed))&&round(m)==='QF'?`예선 ${seed}번 시드`:'',meta=domestic?seedText:[english,seedText].filter(Boolean).join(' · '),mark=logo(t)?`<span class="kvl1180-bracket-mark">${teamLink(t,`<img src="${esc(logo(t))}" alt="${esc(name(t))} 엠블럼" loading="lazy">`,'kvl-team-flag-link')}</span>`:`<span class="kvl1180-bracket-mark is-path ${domestic?'is-school-mark':''}">${esc(domestic?schoolMark(name(t)):'?')}</span>`;return `<div class="kvl1180-bracket-team ${won?'is-winner':''}">${mark}<span class="kvl1180-bracket-copy"><strong>${teamLink(t)}</strong>${meta?`<small>${esc(meta)}</small>`:''}</span><b class="kvl1180-team-score">${sc??'-'}</b></div>`;}
...
function bbye(t,seeds,slot=''){if(!t)return '';const p=participant(t)||t,domestic=String(data().competitionFamily||data().family||'').toLowerCase()==='domestic',prototype=document.body?.dataset?.kvlPrototype==='true',asset=logo(p),k=[code(p),en(p),name(p)].find(x=>seeds.has(x)),seed=k?seeds.get(k):null,english=domestic?'':(en(p)||code(p)||''),mark=asset?`<span class="kvl1180-bracket-mark">${teamLink(p,`<img src="${esc(asset)}" alt="${esc(name(p))} 엠블럼" loading="lazy">`,'kvl-team-flag-link')}</span>`:`<span class="kvl1180-bracket-mark is-path ${domestic?'is-school-mark':''}">${esc(domestic?schoolMark(name(p)):'↗')}</span>`;if(domestic&&prototype)return `<article class="kvl1180-match is-bye is-direct-entry"><div class="kvl1180-match-head"><strong>조 1위 추첨 배정${slot?` (${esc(slot)})`:''}</strong><span>준결승 직행</span></div><div class="kvl1180-bracket-team is-winner">${mark}<span class="kvl1180-bracket-copy"><strong>${teamLink(p)}</strong></span></div><div class="kvl1180-direct-entry-foot">★ 준결승 자동 진출</div></article>`;return `<article class="kvl1180-match is-bye"><div class="kvl1180-match-head"><strong>준결승 직행</strong><span>${seed!=null?`예선 ${esc(seed)}위`:'직행'}</span></div><div class="kvl1180-bracket-team is-winner">${mark}<span class="kvl1180-bracket-copy"><strong>${teamLink(p)}</strong>${english?`<small>${esc(english)}</small>`:''}</span></div></article>`;}
...
function renderKnockout(d){
  ...
  const byNext=new Map();qf.forEach((m,i)=>{const k=m.nextMatchId||m.next_match_id||`SF${Math.floor(i/2)+1}`;if(!byNext.has(k))byNext.set(k,[]);byNext.get(k).push({m,label:m.bracketLabel||`QF${i+1}`});});
  ...
  root.innerHTML=`<div class="kvl1180-bracket"><div class="kvl1180-bracket-ladder"><section class="kvl1180-round kvl1180-round-qf"><h4 class="kvl1180-round-title">${esc(c.qfLabel||'8강 · Quarterfinals')}</h4><div class="kvl1180-round-body">${pairs.map((p,i)=>`<div class="kvl1180-qf-pair">${p.map(x=>bmatch(x.m,x.label,seeds)).join('')}${bbye(direct[i],seeds,i===0?'가':i===1?'나':String(i+1))}</div>`).join('')}</div></section><span class="kvl1180-bracket-gap"></span><section class="kvl1180-round kvl1180-round-sf"><h4 class="kvl1180-round-title">${esc(c.sfLabel||'준결승 · Semifinals')}</h4><div class="kvl1180-round-body">${sf.map((m,i)=>bmatch(m,m.bracketLabel||`SF${i+1}`,seeds)).join('')}</div></section><span class="kvl1180-bracket-gap"></span><section class="kvl1180-round kvl1180-round-final"><h4 class="kvl1180-round-title">${esc(c.finalLabel||'결승 · Final')}</h4><div class="kvl1180-round-body">${bmatch(final,final?.bracketLabel||'FINAL',seeds)}</div></section></div>${bronze?`<div class="kvl1180-bronze"><h4 class="kvl1180-bronze-title">${esc(c.bronzeLabel||'3위 결정전 · Bronze Medal Match')}</h4>${bmatch(bronze,bronze.bracketLabel||'3RD',seeds)}</div>`:''}</div>`;
  renderDomesticRuleCard(d,'knockout',root);
}
function pcard(p){
...
function displayGroups(list,c){
  const domestic=String(data().competitionFamily||data().family||'').toLowerCase()==='domestic';
  if(c.mode==='groups'){
    const map=new Map();list.forEach(p=>{const g=p.group||'참가국';if(!map.has(g))map.set(g,[]);map.get(g).push(p);});
    return [...map.entries()].map(([g,rows])=>[domestic?groupLabel(g):g,domestic?rows.slice().sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''),'ko')):rows]);
  }
