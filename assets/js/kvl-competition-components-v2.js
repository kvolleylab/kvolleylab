...
function side(t,pos){const p=participant(t)||t,asset=logo(p),mark=asset?`<span class="kvl1180-inline-logo">${teamLink(p,`<img src="${esc(asset)}" alt="${esc(name(p))} 엠블럼" loading="lazy">`,'kvl-team-flag-link')}</span>`:`<span class="kvl1180-inline-logo kvl1180-seed-icon">${esc(code(p)||'?')}</span>`,english=en(p)||code(p),copy=`<span class="kvl1180-slot-copy"><strong>${teamLink(p)}</strong>${english?`<small>${teamLink(p,esc(english),'kvl-team-en-link')}</small>`:''}</span>`;return pos==='left'?`<span class="kvl1180-side is-left">${mark}${copy}</span>`:`<span class="kvl1180-side is-right">${copy}${mark}</span>`;}
function matchRow(m){
...
function bbye(t,seeds){if(!t)return '';const p=participant(t)||t,asset=logo(p),k=[code(p),en(p),name(p)].find(x=>seeds.has(x)),seed=k?seeds.get(k):null;return `<article class="kvl1180-match is-bye"><div class="kvl1180-match-head"><strong>준결승 직행</strong><span>${seed!=null?`예선 ${esc(seed)}위`:'직행'}</span></div><div class="kvl1180-bracket-team is-winner">${asset?`<span class="kvl1180-bracket-mark">${teamLink(p,`<img src="${esc(asset)}" alt="${esc(name(p))} 엠블럼" loading="lazy">`,'kvl-team-flag-link')}</span>`:'<span class="kvl1180-bracket-mark is-path">↗</span>'}<span class="kvl1180-bracket-copy"><strong>${teamLink(p)}</strong><small>${esc(en(p)||code(p)||'')}</small></span></div></article>`;}
