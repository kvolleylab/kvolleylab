...
function teamCell(t,cls){const english=en(t)||code(t);return `<span class="${cls}">${flag(t)?teamLink(t,`<img src="${esc(flag(t))}" alt="${esc(name(t))} 국기" loading="lazy">`,'kvl-team-flag-link'):''}<span class="${cls}-copy"><strong>${teamLink(t)}</strong><small>${teamLink(t,esc(english),'kvl-team-en-link')}</small></span></span>`;}
...
function bteam(m,key,seeds){const t=key==='home'?m.home:m.away,s=score(m),won=!!winner(m)&&name(winner(m))===name(t),k=[code(t),en(t),name(t)].find(x=>seeds.has(x)),seed=k?seeds.get(k):null,sc=key==='home'?s?.home:s?.away,english=en(t)||code(t)||'대진',seedText=seed!=null&&Number.isFinite(Number(seed))&&round(m)==='QF'?` · 예선 ${seed}번 시드`:'';return `<div class="kvl1180-bracket-team ${won?'is-winner':''}">${flag(t)?`<span class="kvl1180-bracket-mark">${teamLink(t,`<img src="${esc(flag(t))}" alt="${esc(name(t))} 국기" loading="lazy">`,'kvl-team-flag-link')}</span>`:`<span class="kvl1180-bracket-mark is-path">?</span>`}<span class="kvl1180-bracket-copy"><strong>${teamLink(t)}</strong><small>${teamLink(t,esc(english),'kvl-team-en-link')}${esc(seedText)}</small></span><b class="kvl1180-team-score">${sc??'-'}</b></div>`;}
