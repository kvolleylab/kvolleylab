...
function renderRoster(d,teamCode){
 ...
 const rows=players.map(p=>`<div class="kvl1180-roster-row" role="row" data-player-id="${esc(p.playerId||'')}"><span class="kvl1180-roster-no">${esc(p.number??'-')}</span><span class="kvl1180-roster-name"><strong>${esc(p.en||'-')}</strong></span><span class="kvl1180-roster-korean">${esc(p.name||'-')}</span><span class="kvl1180-roster-pos">${esc(p.position||'-')}</span><span class="kvl1180-roster-dob">${esc(p.dob||'-')}</span><span class="kvl1180-roster-height">${p.heightCm>0?esc(p.heightCm)+'cm':'키 확인 불가'}</span><span class="kvl1180-roster-club" data-season-label="${esc(d.clubSeasonLabel||'소속팀')} · "><span class="kvl1180-club-main">${esc(p.club?.name||'미확인')}</span>${p.club?.country?`<span class="kvl1180-club-country">${p.club.flag?`<img src="${esc(p.club.flag)}" alt="">`:''}<span>${esc(p.club.country)}${p.club.league?' · '+esc(p.club.league):''}</span></span>`:''}</span><span class="kvl1180-roster-vb">${p.volleybox?`<a class="kvl1180-vb-link" href="${esc(p.volleybox)}" target="_blank" rel="noopener noreferrer" aria-label="${esc(p.name)} Volleybox 프로필"><span>VB</span><b>↗</b></a>`:'—'}</span><span data-player-action-slot="${esc(p.playerId||'')}" hidden></span></div>`).join('');
 root.innerHTML=`<article class="kvl1180-team-profile"><header class="kvl1180-team-profile-head"><span class="kvl1180-team-profile-flag"><img src="${esc(t?.flag||'')}" alt="${esc(t?.name||'')} 국기"></span><div class="kvl1180-team-profile-copy"><p class="label">SELECTED TEAM · ${esc(teamCode)}</p><h3>${esc(t?.name||teamCode)}</h3><p>${esc(t?.en||'')}</p></div><div class="kvl1180-team-profile-meta"><span>${esc(t?.group||'')}</span><span>${esc(r.statusLabel||'등록 '+players.length+'명')}</span></div></header><div class="kvl1180-roster-block"><div class="kvl1180-roster-headline"><div><h4>등록 선수명단</h4><p>${esc(d.rosterNote||'대회 당시 등록 로스터 Snapshot')}</p></div></div><div class="kvl1180-roster-table" role="table" aria-label="${esc(t?.name)} 등록 선수명단"><div class="kvl1180-roster-table-head" role="row"><span>등번호</span><span>영문명</span><span>한글명</span><span>POS</span><span>생년월일</span><span>키</span><span>${esc(d.clubSeasonLabel||'소속팀')} · 리그</span><span>VB</span></div>${rows}</div></div></article>`;
}
