...
function apply(){
 ...
 observer?.disconnect();
 const rows=[...history.querySelectorAll('.pd-history-row')];if(!rows.length){observer?.observe(area,{childList:true,subtree:true});return}
...
observer=new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(apply,0)});observer.observe(area,{childList:true,subtree:true});apply();
})();