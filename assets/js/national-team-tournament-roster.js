(()=>{
  ...
  function renderPlayerTable(team){
    const rows=(team.players||[]).map(p=>{
      ...
      return `<tr><td class="ntroster-no">${display(p.number)}</td><td class="ntroster-name"><strong>${esc(p.name||'—')}</strong>${nameKo}</td><td><span class="ntroster-position">${esc(positionText(p.positions))}</span></td><td>${display(p.dob)}</td><td>${p.height_cm?`${esc(p.height_cm)}cm`:'—'}</td><td>${vb}</td></tr>`;
    }).join('');
  ...
  async function loadAvcMenCup(){
    ...
    if(imported)renderPlayerTable(roster);else{players.className='ntroster-empty';players.innerHTML='이 국가의 공식 명단은 검수를 마쳤으며 사이트 데이터 반영을 진행 중입니다.';}
    const sourceLinks=[];
  ...
  (isAvcMenCup?loadAvcMenCup():loadContinental()).catch(()=>{
    ...
    status.textContent='불러오기 실패';status.className='ntroster-status pending';
  });
})();