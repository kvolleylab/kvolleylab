...
function simplifySources(){
  ...
  if(!section)return;
  const note=section.querySelector(':scope > .sc-title > p');
  if(note)note.textContent='공식 원본 자료';
  ...
  let root=section.querySelector(':scope > .sc-sources');
  if(!root){
    root=document.createElement('div');
    root.className='sc-sources';
