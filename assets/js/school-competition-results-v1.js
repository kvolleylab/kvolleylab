(()=>{
  ...
  function render(){
    ...
    const groups=dates.map(date=>{
      ...
      return `<section class="scr-date-section"><div class="scr-date-head"><h2>${fmtDate(date)}</h2><span>${matches.length}경기</span></div><div class="scr-grid">${matches.map(m=>`<article class="scr-card"><div class="scr-card-top"><span>${esc(m.venue)} · ${m.court_order}경기</span><span class="scr-stage">${esc(m.stage)}</span></div><div class="scr-match"><strong class="scr-team ${m.winner===m.team_a?'winner':''}">${esc(m.team_a)}</strong><b class="scr-score">${esc(m.set_score)}</b><strong class="scr-team away ${m.winner===m.team_b?'winner':''}">${esc(m.team_b)}</strong></div><div class="scr-sets">${m.sets.map((s,i)=>`<span>${i+1}세트 ${esc(s)}</span>`).join('')}</div><div class="scr-source">${esc(m.division)}${m.source_section?` · ${esc(m.source_section)}`:''}</div></article>`).join('')}</div></section>`;
    }).join('');
  ...
  fetch(`data/domestic/${encodeURIComponent(id)}.json?v=20260724-1`,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('data');return r.json()}).then(d=>{
    data=d;
