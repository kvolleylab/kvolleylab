(()=>{
  ...
  function calendarFilters(){
    ...
    return `<div class="scr-calendar-filters"><div><span class="scr-filter-label">부문</span>${buttons(divisions,activeDivision,'division','전체 부문')}</div><div><span class="scr-filter-label">단계</span>${buttons(stages,activeStage,'stage','전체 단계')}</div></div>`;
  }
  ...
  function calendar(){
    ...
    return `<section class="scr-calendar-wrap"><div class="scr-calendar-head"><div><h2>${start.getFullYear()}년 ${start.getMonth()+1}월</h2><p>부문과 단계를 선택해 달력 일정을 좁혀볼 수 있습니다.</p></div><span>${rows.length}경기 표시</span></div>${calendarFilters()}<div class="scr-weekdays"><span>일</span><span>월</span><span>화</span><span>수</span><span>목</span><span>금</span><span>토</span></div><div class="scr-calendar">${cells.join('')}</div></section>`;
  }
  ...
  function render(){
    const rows=filtered(),dates=uniq(data.matches.map(m=>m.date)).sort(),divisions=uniq(data.matches.map(m=>m.division)).sort(),stages=uniq(data.matches.map(m=>m.stage)),venues=uniq(data.matches.map(m=>m.venue));
    const listFilters=view==='list'?`<div class="scr-list-filters">${calendarFilters()}</div>`:'';
    app.innerHTML=`<div class="scr-view-switch"><button class="${view==='calendar'?'active':''}" data-view="calendar">월간 달력</button><button class="${view==='list'?'active':''}" data-view="list">전체 목록</button></div>${view==='calendar'?calendar():listFilters}<div class="scr-date-toolbar"><span class="scr-filter-label">날짜별 경기</span>${buttons(dates,activeDate,'date','전체 날짜')}</div><div class="scr-summary"><div class="scr-kpi"><span>전체 경기</span><strong>${data.matches.length}</strong></div><div class="scr-kpi"><span>현재 표시</span><strong>${rows.length}</strong></div><div class="scr-kpi"><span>경기장</span><strong>${venues.length}</strong></div><div class="scr-kpi"><span>부문</span><strong>${divisions.length}</strong></div></div><div id="scrDetail">${details(rows)||'<div class="scr-empty">선택한 조건에 해당하는 경기가 없습니다.</div>'}</div><div class="scr-note">${data.notes.map(x=>`• ${esc(x)}`).join('<br>')}</div><a class="scr-back" href="domestic-competitions.html?division=school">← 국내 중·고 대회 목록</a>`;
    bind();
  }
