(()=>{
  ...
  const fmtMetaDate=s=>{const d=new Date(`${s}T00:00:00`);return `${s}(${weekday[d.getDay()]})`};
  ...
  const stageOrder=['사전경기','예선','8강','6강','준결승','4강','결승'];
  const labelStage=s=>s==='준결승'?'4강':s;
  const sortByOrder=(items,order)=>items.slice().sort((a,b)=>{const ai=order.indexOf(a),bi=order.indexOf(b);return (ai<0?99:ai)-(bi<0?99:bi)||String(a).localeCompare(String(b),'ko')});
  ...
  const calendarFiltered=()=>data.matches.filter(m=>(activeDivision==='all'||m.division===activeDivision)&&(activeStage==='all'||m.stage===activeStage));
  const filterButtons=(items,active,key,labelAll,labeler=v=>v)=>`<div class="scr-filter-row"><button class="scr-filter ${active==='all'?'active':''}" data-${key}="all">${labelAll}</button>${items.map(v=>`<button class="scr-filter ${active===v?'active':''}" data-${key}="${esc(v)}">${esc(labeler(v))}</button>`).join('')}</div>`;
  const matchCard=m=>`<article class="scr-card"><div class="scr-card-top"><span>${esc(m.venue)} · ${m.court_order}경기</span><span class="scr-stage">${esc(labelStage(m.stage))}</span></div><div class="scr-match"><strong class="scr-team ${m.winner===m.team_a?'winner':''}">${esc(m.team_a)}</strong><b class="scr-score">${esc(m.set_score)}</b><strong class="scr-team away ${m.winner===m.team_b?'winner':''}">${esc(m.team_b)}</strong></div><div class="scr-sets">${m.sets.map((s,i)=>`<span>${i+1}세트 ${esc(s)}</span>`).join('')}</div><div class="scr-source">${esc(m.division)}</div></article>`;

  function controlPanel(rows){
    const divisions=sortByOrder(uniq(data.matches.map(m=>m.division)),divisionOrder);
    const stages=sortByOrder(uniq(data.matches.map(m=>m.stage)),stageOrder);
    return `<section class="scr-master-controls"><div class="scr-master-row"><div class="scr-view-switch"><button class="${view==='calendar'?'active':''}" data-view="calendar">월간 달력</button><button class="${view==='list'?'active':''}" data-view="list">전체 목록</button></div><div class="scr-inline-filter"><span>부문</span>${filterButtons(divisions,activeDivision,'division','전체 부문')}</div></div><div class="scr-master-row scr-master-row-bottom"><strong>${rows.length}경기 표시</strong><div class="scr-inline-filter"><span>단계</span>${filterButtons(stages,activeStage,'stage','전체 단계',labelStage)}</div></div></section>`;
  }
  ...
  function calendar(){
    ...
    const rows=calendarFiltered(),cells=[];
    for(let d=new Date(gridStart);d<=gridEnd;d.setDate(d.getDate()+1)){
      const key=iso(d),matches=rows.filter(m=>m.date===key),outside=d.getMonth()!==monthStart.getMonth();
    ...
    return `<section class="scr-calendar-wrap"><div class="scr-calendar-head"><h2>${start.getFullYear()}년 ${start.getMonth()+1}월</h2></div><div class="scr-weekdays"><span>일</span><span>월</span><span>화</span><span>수</span><span>목</span><span>금</span><span>토</span></div><div class="scr-calendar">${cells.join('')}</div></section>`;
  }
  ...
  function render(){
    const rows=filtered(),dates=uniq(data.matches.map(m=>m.date)).sort();
    app.innerHTML=`${controlPanel(calendarFiltered())}${view==='calendar'?calendar():''}<section class="scr-date-toolbar"><span class="scr-filter-label">날짜별 경기</span>${filterButtons(dates,activeDate,'date','전체 날짜',fmtDate)}</section><div id="scrDetail">${details(rows)||'<div class="scr-empty">선택한 조건에 해당하는 경기가 없습니다.</div>'}</div><div class="scr-note">${data.notes.map(x=>`• ${esc(x)}`).join('<br>')}</div><a class="scr-back" href="domestic-competitions.html?division=school">← 국내 중·고 대회 목록</a>`;
    bind();
  }

  function bind(){
    app.querySelectorAll('[data-date]').forEach(b=>b.onclick=()=>{activeDate=b.dataset.date;render()});
    app.querySelectorAll('[data-division]').forEach(b=>b.onclick=()=>{activeDivision=b.dataset.division;activeDate='all';render()});
    app.querySelectorAll('[data-stage]').forEach(b=>b.onclick=()=>{activeStage=b.dataset.stage;activeDate='all';render()});
    app.querySelectorAll('[data-view]').forEach(b=>b.onclick=()=>{view=b.dataset.view;activeDate='all';render()});
  ...
  fetch(`data/domestic/${encodeURIComponent(id)}.json?v=20260726-1`,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('data');return r.json()}).then(d=>{
    ...
    document.getElementById('scrTitle').textContent=d.competition.startsWith('2026')?d.competition:`2026 ${d.competition}`;
    document.getElementById('scrMeta').textContent=`${fmtMetaDate(d.start)} ~ ${fmtMetaDate(d.end)} · ${d.location} · 총 ${d.matches.length}경기`;
    render();
