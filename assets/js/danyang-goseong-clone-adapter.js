...
window.fetch=async(input,init)=>{
  ...
  if(url.includes('data/competitions/gosung-2026.json')){
    const response=await nativeFetch(`${DANYANG_URL}?v=20260731-10`,init);
...
function applyStatic(data){
  ...
  document.querySelectorAll('.cd-jump a[data-view]').forEach(a=>a.href=`university-competition-danyang.html?view=${a.dataset.view}`);
  ...
  const finalLink=document.querySelector('.cd-jump a[data-view="standings"]');
  if(finalLink)finalLink.hidden=true;
  ...
  if(qualifier)qualifier.hidden=true;
  const playerCount=document.getElementById('cdPlayerCount'),avg=document.getElementById('cdAvgHeight'),note=document.getElementById('cdSnapshotNote');
...
async function start(){
  try{
    const response=await nativeFetch(`${DANYANG_URL}?v=20260731-10`,{cache:'no-store'});
    ...
    const timer=setInterval(()=>{
      ...
      if(document.querySelector('.cd-match')||attempts>=30)clearInterval(timer);
    },100);
