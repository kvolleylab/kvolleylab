...
Promise.all([
 ...
 const teamName=teamPlayers[0].school;const teamCode=teamPlayers[0].code;document.title=`${teamName} | K-Volley Lab`;
 const heights=teamPlayers.map(p=>p.height).filter(Boolean);const grades=Object.fromEntries(GRADE_KEYS.map(k=>[k,0]));const positions=Object.fromEntries(POSITION_KEYS.map(k=>[k,0]));teamPlayers.forEach(p=>{if(GRADE_KEYS.includes(p.grade))grades[p.grade]+=1;if(p.position in positions)positions[p.position]+=1});
 const rawStaff=(staffData.teams||{})[teamCode]||{};const headCoach=rawStaff.head_coach||'확인 중';const coaches=Array.isArray(rawStaff.coaches)&&rawStaff.coaches.length?rawStaff.coaches.join(', '):'없음';
