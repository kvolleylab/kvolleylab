...
function renderStageFilters(list){
 ...
 const wrap=$('cdStageFilters');
 if(!wrap)return;
 wrap.innerHTML=stages.map(stage=>`<button type="button" class="${stage===stageFilter?'is-active':''}" data-stage="${esc(stage)}">${esc(stage)}</button>`).join('');
 wrap.querySelectorAll('button').forEach(btn=>btn.addEventListener('click',()=>{stageFilter=btn.dataset.stage;renderResults()}));
}
...
function renderStandings(){
 const target=$('cdGroupStandings');
 if(!target)return;
 const prelim=competition.games.filter(g=>g.division===division&&g.pool);
 const pools=[...new Set(prelim.map(g=>g.pool))].sort();
 if(!pools.length){target.innerHTML='<div class="cd-empty">조별리그 순위 데이터가 없습니다.</div>';return}
 target.innerHTML=pools.map(pool=>{
 ...
 const note=$('cdStandingNote');
 if(note)note.textContent=`${division} 조별리그 경기결과를 승수·세트득실 순으로 정리한 참고 순위입니다.`;
}
...
function markHtml(team){
 ...
 return `<span class="cd-team-mark" title="${brand?.status==='source_verified'?'공식 출처 확인 완료 · 원본 파일 반영 대기':'공식 로고 확인 대기'}">${esc(initials(team.name))}</span>`;
}
