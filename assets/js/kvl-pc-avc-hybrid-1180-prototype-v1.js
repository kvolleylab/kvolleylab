...
function poolCard(pool,rows,qualifierSet){
  const body=rows.map(r=>{const qualified=qualifierSet.has(r.team),korea=r.team==='대한민국';return `<div class="kvl1180-pool-row ${qualified?'is-qualified':'is-out'} ${korea?'is-korea':''}"><span class="kvl1180-pool-rank">${r.poolPosition}</span>${teamIdentity(r.team,'kvl1180-pool')}<span class="kvl1180-pool-w">${r.wins}</span><span class="kvl1180-pool-l">${r.losses}</span><span class="kvl1180-pool-pts">${r.matchPoints}</span><span class="kvl1180-pool-ratio">${fmtRatio(r.setsFor,r.setsAgainst)}</span><span class="kvl1180-pool-ratio">${fmtRatio(r.pointsFor,r.pointsAgainst)}</span></div>`}).join('');
  return `<article class="kvl1180-pool-card"><header class="kvl1180-pool-card-head"><strong>${esc(pool)}조</strong><span>4개국 · 6경기</span></header><div class="kvl1180-pool-table-head"><span>순위</span><span>국가</span><span>승</span><span>패</span><span>승점</span><span>세트</span><span>득점</span></div>${body}</article>`;
}
