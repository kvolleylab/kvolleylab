...
async function setupDivisionTabs(){const podium=document.getElementById('cdPodium'),teams=document.getElementById('cdTeams'),knockout=document.querySelector('#standings .cd-knockout-wrap');if(!podium||!teams||!knockout||document.querySelector('.cd-division-switch'))return;let data;try{data=await fetch('data/competitions/gosung-2026.json',{cache:'no-store'}).then(r=>r.json())}catch{return}
const malePodium=podium.innerHTML,maleTeams=teams.innerHTML,maleBracket=knockout.querySelector('#cdKnockoutBracket').innerHTML;
