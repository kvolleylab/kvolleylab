...
const officialUrl=g=>g.official_match_id?`https://en.volleyballworld.com/volleyball/competitions/avc-men-cup/schedule/${g.official_match_id}/`:'#';
const scoreText=g=>g.status==='completed'&&g.result?`${g.result.home_sets}-${g.result.away_sets}`:'vs';
...
const stageAllowed=g=>!state.stage||g.stage_group===state.stage;
...
function filtersHtml(){
  const stageBtns=[[0,'전체'],[1,'예선·순위전'],[4,'파이널 라운드']].map(([v,l])=>`<button class="sv3-btn ${state.stage===v?'active':''}" data-stage="${v}">${l}</button>`).join('');
  ...
  return `<div class="sv3-filter-title">단계 선택</div><div class="sv3-week-buttons">${stageBtns}</div><div class="sv3-filter-title">개최 지역</div><div class="sv3-places"><div class="sv3-place-row ${VENUE.cls}"><span class="sv3-dot"></span><span class="sv3-place-label">${venueLabel()}</span></div></div>${sectionTitle('출전 국가','teams',state.teamsOpen)}<div class="sv3-collapsible ${state.teamsOpen?'open':''}"><button class="sv3-select-all ${state.teams.size===allTeams.length?'active':''}" data-toggle-teams>${summaryText(state.teams.size,allTeams.length)}</button><div class="sv3-places sv3-team-list">${teamRows}</div></div><div class="sv3-filter-title">시간 기준</div><div class="sv3-time-row"><button class="sv3-btn ${state.timeMode==='kst'?'active':''}" data-time="kst">한국시간</button><button class="sv3-btn ${state.timeMode==='local'?'active':''}" data-time="local">인도 현지시간</button></div><div class="sv3-time-note">아메다바드 현지시간은 한국시간보다 3시간 30분 느립니다.</div><div class="sv3-filter-actions"><button class="sv3-done" id="sv3Done">완료</button><button class="sv3-reset" id="sv3Reset">필터 초기화</button></div>`;
}
...
function gameMini(g){const middle=scoreText(g);return `<a class="sv3-game ${VENUE.cls}" href="${officialUrl(g)}" target="_blank" rel="noopener noreferrer" aria-label="${g.home} ${middle} ${g.away} 공식 경기"><div class="sv3-game-line"><span class="sv3-game-time">${displayTime(g)}</span><span class="sv3-game-match">${g.home} ${middle} ${g.away}</span></div><span class="sv3-game-place">${venueLabel()}</span></a>`;}
function detailHtml(){let day=state.selectedDay;if(!day){day=Array.from({length:month.days},(_,i)=>i+1).find(d=>filteredGames(d).length)||20;state.selectedDay=day;}const games=filteredGames(day);return `<div class="sv3-detail-head"><h3>2026-06-${String(day).padStart(2,'0')} 경기 일정</h3><strong>총 ${games.length}경기</strong></div>${games.length?venueCard(games):'<div>선택 조건에 맞는 경기가 없습니다.</div>'}`;}
function venueCard(gs){return `<section class="sv3-venue-card ${VENUE.cls}"><div class="sv3-venue-title">${venueLabel()}</div>${gs.map(g=>`<a class="sv3-match-row" href="${officialUrl(g)}" target="_blank" rel="noopener noreferrer"><time>${displayTime(g)}</time><div class="sv3-result-line"><div class="sv3-team-block">${flagImg(g.home)}<span class="sv3-team-name">${g.home}</span></div><strong class="sv3-result-score">${scoreText(g)}</strong><div class="sv3-team-block"><span class="sv3-team-name">${g.away}</span>${flagImg(g.away)}</div></div></a>`).join('')}</section>`;}
function bind(){
...
fetch('data/matches/avc-2026-calendar.json?v=20260816-2',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('schedule load failed');return r.json();}).then(data=>{
  matches=(data.matches||[]).filter(x=>x&&x.match_id&&x.home&&x.away);
