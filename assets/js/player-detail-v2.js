(()=>{
  ...
  const observationUrl='data/observations/player_observations_v1.json';
  ...
  Promise.all([
  ...
  ]).then(([players,timelines,observations])=>{
    ...
    const playerId=player?.system?.player_id;
    const timeline=timelines.find(t=>t.player_id===playerId)?.events||[];
    const notes=observations.filter(o=>o.player_id===playerId&&o.visibility==='public'&&o.status!=='deleted');
    render(player,timeline,notes);
  ...
  function renderObservations(notes){if(!notes.length)return '<div class="pd-empty">공개 승인된 관찰 메모가 아직 없습니다.</div>';return[...notes].sort((a,b)=>String(b.observed_at||'').localeCompare(String(a.observed_at||''))).map(note=>`<div class="pd-observation"><div class="pd-observation-head"><strong>${esc(val(note.observed_at,'날짜 미상'))}</strong>${note.competition_id?`<span>${esc(note.competition_id)}</span>`:''}</div>${note.summary?`<p>${esc(note.summary)}</p>`:''}${(note.strengths||[]).length?`<div><b>강점</b> · ${esc(note.strengths.join(' · '))}</div>`:''}${(note.improvements||[]).length?`<div><b>보완</b> · ${esc(note.improvements.join(' · '))}</div>`:''}${(note.tags||[]).length?`<div class="pd-observation-tags">${note.tags.map(tag=>`<span>${esc(tag)}</span>`).join('')}</div>`:''}</div>`).join('')}

  function render(p,timelineEvents,observations){
