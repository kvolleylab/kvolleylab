...
V2는 특정 실제 대회 페이지를 복제하는 방식이 아니라 **공통 구조 + 성별 테마 + 대회 데이터 + 대회 상태(upcoming/active/completed)** 를 분리한 재사용 기준이다. V1은 역사적 동결 스냅샷으로 보존하고 신규 국제대회는 V2를 우선한다.
...
- 재사용 smoke test: `tests/competition-page-v2-reuse-smoke.html`
...
- MEN/WOMEN 전환은 동일 단계끼리 연결한다. 프로토타입에서 production으로 빠지지 않는다.
...
V2에서는 `data-kvl-gender="men|women"`만으로 테마를 결정한다. 여자부가 남자부 선택자를 재사용하기 위해 `data-avc-gender="men"` 같은 우회 속성에 의존하지 않는다.
...
- 우승 확정 전 한눈에 보기 우측 안내 기본값은 `조별리그 결과에 따라 예상 8강 대진이 자동 반영됩니다.`이다. 조별리그 이후에는 같은 의미의 단계별 안내로 데이터에서 교체할 수 있다.
...
- 결과 묶음 왼쪽 3px 세로선은 PC/모바일 모두 유지하며 성별 테마색을 사용한다.
...
- 여자부 Rose / 남자부 Green, 특수 성취만 Gold.
...
- 실제 대회 고유 국가명·팀수·경기수
- AVC 전용 URL
- 여자부 fallback/guard 등 특정 대회 복구코드
- pseudo-element에 하드코딩한 우승팀/진출팀

## 신규 대회 적용 순서
1. V2 HTML/데이터 예시 복제
2. competition ID·메타 입력
3. `status`·`stageLabel` 입력
4. 성별 테마 선택
5. Hero PC/MOBILE 연결
6. 참가국/조편성/일정 연결
